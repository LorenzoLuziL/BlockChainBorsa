...
//Check if i can execute that message 
    function checkTheExecution(bytes32 idMessage,bytes32 hashInstance) private returns (bool){
        ...
        if(temp.messageIn==idMessage){
            if(istancies[hashInstance].controlFlowElementList[temp.idInElement].id!=bytes32(0) && istancies[hashInstance].controlFlowElementList[temp.idInElement].elementType==ElementType.START){
...
function checkNextElement(bytes32 idElement,bytes32 hashInstance) private {
    ...
    if (istancies[hashInstance].controlFlowElementList[idElement].id != bytes32(0) ) {
        ...
        if(checkForNextGatewayCondition(idElement,hashInstance) && istancies[hashInstance].controlFlowElementList[idElement].elementType!=ElementType.EX_SPLIT ){
